...
local ValueString = class(Value)

function ValueString:init(parent,config)
	ValueString.super.init(self,parent,config)
	config.noArrow = not (config.selection and #config.selection ~= 0)
	self:_initLayout()
	self:val(config.value or '')
	self.box = self.pnl:rect{color = cl.White:with_alpha(0.3), visible = false}
	self
	:on('press',function(b,x,y)
		if b == 0 then
...
function ValueString:selection()
	return self.config.selection or {}
end

function ValueString:prev()
	ValueChoose.go(self,-1)
	self._lastClick = now()
end

function ValueString:innerVal(set)
	if set then
...
function ValueString:next()
	ValueChoose.go(self,1)
	self._lastClick = now()
end

function ValueString:_initLayout()
	if not self.config.valX and self.config.text:gsub(' ','') == '' then
		self.config.valX = self.config.w / 2
	end
end

function ValueString:val(set)
	if set then
		set = utf8.sub(set,1,self.config.max or 15)
	end
	local result =  ValueString.super.val(self,set)
	self:repaint()
	if set then
...
function ValueString:startEdit()
	self._editing = true
...
function ValueString:selectAll()
...
function ValueString:_select(tS,tE)
...
function ValueString:select(delta,shift)
...
function ValueString:_setCaret(worldX)
...
function ValueString:endEdit(cancel)
	self._editing = nil
	ROOT._focused = nil
...
function ValueString:enter_text(o, s)
...
function ValueString:key_release(o, k)
...
function ValueString:key_press(o, k)
